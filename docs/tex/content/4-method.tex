...
We start by deciding on an EEG device to use, we then collect data for both the controlled and naturalistic experiments. For the controlled experiment, we collect stimuli labels. For the uncontrolled experiment, we collect labels using ActivityWatch. Once the data has been collected, we perform data labelling, transformation, and cleaning, before we finally train our classifiers. 
