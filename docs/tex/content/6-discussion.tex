...
We consider our novel idea of `mental churn', a neurally integrated alternative to the measure of code churn, where not only code changes are measured as churn, but also the \emph{attention} a file or piece of code receives as a whole. The notion of mental churn could also include what response the code elicited in an engineer (confusion, focus, confidence).
...
\section{Crowdsourcing data}
    ...
    A potential solution to the problem of learning new variations from a small additional sample problem is to attempt cross-task or cross-subject transfer learning. In such a setup, a model is already trained on a large amount of subjects, or many tasks, and through additional collected data can self-improve and adapt to one user or one task. This is discussed further in Section~\ref{section:transfer-learning}.

    \add[inline]{Write about crowdsourcing of EEG data, including the potential of transfer learning and privacy considerations.}

    \paragraph*{NeuroTech Challenge Series}\label{section:ntcs}

    As part of the thesis work I have contributed to an effort in crowdsourcing EEG data collected from the experiments built in eeg-notebooks, using mobile consumer EEG devices like those from Muse, OpenBCI, and Neurosity\@. The effort, called the \href{https://neurotech-challenge.com/}{NeuroTech Challenge Series} (NTCS), is a collaborative effort lead by John Griffiths at the University of Toronto, with support from OpenBCI and NeuroTechX.
